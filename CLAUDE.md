...
- **engagement — mise en avant (+5)** : migration `35b` (`engagement.mise_en_avant`, table générique) câblant la règle `contribution_mise_en_avant`. Back-office `POST/GET/DELETE /admin/engagement/mise-en-avant` (+5 idempotent, anti-auto-attribution, pas de clawback) ; `AdminMiseEnAvantBouton` dans les 5 pages admin (codimoi/factcheck/bad-habits/idea-forces/vidafrica-piste). Cf. gamification Phase 1 (`35_engagement.sql`).
