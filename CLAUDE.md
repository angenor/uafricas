...
- **afrolang — espace commentaires temps réel en session** : aucune migration ni route neuve (`afrolang.message_session` + `GET/POST /sessions/{id}/messages` dormaient depuis la feature 005, `SalleChat.vue` n'était monté nulle part). Le composant est refondu : le **POST persiste puis le client émetteur diffuse le message canonique** (avec son id) en DataPacket LiveKit `type:'chat'` `reliable:true` — dédoublonnage par id, la diffusion ne recréant jamais un identifiant local. Deux filets, car le temps réel dépend du client émetteur : rattrapage `?since=` sur `RoomEvent.Reconnected` **et** sondage de 30 s (`document.hidden` respecté) si son `publishData` a échoué. Monté dans `AfrolangRoom` en `v-show` — replié, il continue d'écouter, sinon le compteur de non-lus serait faux ; les deux pages session (publique et privée) en héritent sans changement.
