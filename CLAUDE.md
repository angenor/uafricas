...
- Rust Edition 2024 (backend) + TypeScript / Nuxt 4 / Vue 3 SSR (frontend) + Actix-Web 4, sqlx (PostgreSQL), uuid, chrono, serde, **livekit-api** (déjà présent — utilisé pour token signing aujourd'hui, étendu pour `RoomServiceClient::update_participant`) ; Pinia, $fetch, FontAwesome, **livekit-client** (déjà présent côté Vue pour `DataPacket_Kind.RELIABLE`) (001-session-moderation)
- PostgreSQL 16 — schema `afrolang` existant (1 nouvelle table + 3 colonnes sur `session`) (001-session-moderation)

## Recent Changes
- 001-ressources-fermeture-session (Phases 4→6 livrées) : fermeture administrative pour abus + historique de modération. BDD : 6 colonnes désactivation/réactivation ajoutées à `afrolang.salle` + table append-only `afrolang.evenement_moderation_salle` (déjà créées en Phase 2). Backend : nouveau handler `admin/sessions_moderation.rs` avec 3 endpoints (`POST /api/admin/afrolang/sessions/{id}/fermer-admin` — transaction `session.etat='terminee'` + `salle.desactivee_admin_*` + insert évènement, hors transaction `livekit_moderation::fermer_session_admin` + notifications participants `afrolang.session.fermee_admin` et admins de salle `afrolang.salle.desactivee_admin`, double audit ; `POST /api/admin/afrolang/salles/{id}/reactiver` — transaction inverse + notification `afrolang.salle.reactivee_admin` ; `GET /api/admin/afrolang/salles/{id}/historique-moderation` paginé avec JOIN admin). `SALLE_COLONNES` + `SalleRow` enrichis des 6 colonnes désactivation, `to_response()` publique masque le motif (FR-020), `to_desactivation_admin()` l'expose côté admin. Contrôle 403 `salle_desactivee_admin` ajouté aux 3 endpoints de jointure (`demarrer_ou_rejoindre_session_salle_publique`, `verifier_code_acces_salle_privee`, `demarrer_ou_rejoindre_session_salle_privee`). Frontend : `useAdminSessions.fermerSessionAdmin`, `useAdminAfrolangSalles.reactiverSalle` / `listerHistoriqueModeration` ; nouveaux composants admin daisyUI `SessionFermetureModal.vue` (compteur 10/1000), `SalleReactivationModal.vue` (commentaire ≤1000), `SalleHistoriqueModerationPanel.vue` (table chronologique badges colorés) ; composant public Tailwind v4 pur `SalleDesactiveeBadge.vue` (bordure rouge + icône `faBan`) ; intégration dans `pages/admin/salles/[id].vue` (nouvel onglet « Historique de modération » + bandeau de désactivation + bouton « Réactiver ») et `components/afrolang/SalleCard.vue` (badge compact en haut à droite + bandeau désactivation + bouton « Démarrer/Rejoindre » désactivé). FontAwesome : `faGavel` ajouté.
- 001-session-moderation (Phases 1→6 livrées) : modération de session Afrolang — permissions tableau blanc individuelles + mise en évidence (spotlight) en sessions publiques livestreamées. BDD : nouvelle table `afrolang.session_permission_tableau_blanc` (PK composite `(session_id, utilisateur_id)`, FK CASCADE, 2 indexes) + 3 colonnes nullables `participant_mis_en_evidence_id`/`mis_en_evidence_par`/`mis_en_evidence_at` sur `afrolang.session` avec CHECK `ck_session_spotlight_coherent`. Backend : helper `est_moderateur_session` (4 niveaux — admin plateforme, admin salle, modérateur attitré, créateur salle privée) + enum `NiveauModerateur::peut_spotlight()` (FR-001b — modérateur attitré exclu) ; nouveau module `services/livekit_moderation.rs` (`update_participant_can_publish_data` via `RoomServiceClient::update_participant` + `publier_evenement_moderation` via `send_data` RELIABLE) ; 5 endpoints REST (`GET|POST|DELETE /api/afrolang/sessions/{id}/permissions-tableau-blanc[/{user_id}]` + `POST|DELETE /api/afrolang/sessions/{id}/spotlight`) ; enforcement LiveKit `can_publish_data` à la jointure (`generer_token_session`) + nettoyage `DELETE FROM session_permission_tableau_blanc` à la transition `etat='terminee'` (FR-017) + cascade spotlight à `NULL` quand le mis en vedette quitte la session (FR-025) ; `obtenir_session` enrichi de `spotlight: SpotlightInfo | null` + `permissions_tableau_blanc_count` (FR-024). DataPackets LiveKit `{type:'moderation', subtype:'permission_update'|'spotlight'}`. audit::log_action sur toutes mutations. Frontend (Tailwind v4 pur — principe VI) : composable `useAfrolang` étendu (interfaces `PermissionTableauBlancAPI`, `SpotlightInfoAPI`, `NiveauModerateur`, états `monNiveauModerateurSession`/`permissionsTableauBlanc`/`spotlightActif`/`monEcritureAutorisee` computed, méthodes `lister|accorder|retirerPermissionTableauBlanc` + `mettre|retirerMiseEnEvidence`, listener LiveKit `attacherListenerModeration` avec callbacks `onPermissionUpdate`/`onSpotlight`) ; nouveau composant `SalleModerationPanel.vue` (section permissions TB avec toggle par participant + section spotlight conditionnelle admin+publique) ; `AfrolangWhiteboard.vue` accepte prop `ecritureAutorisee` (overlay « Lecture seule » + postMessage `set-readonly` à l'iframe Excalidraw) ; `AfrolangVideoGrid.vue` bascule en disposition spotlight (tuile cible agrandie border-2 border-custom-chocolat + badge « En vedette » FontAwesome `faStar`, miniatures en bas, transition 300 ms) ; `AfrolangRoom.vue` initialise `spotlightActif` depuis le GET session au mount (FR-024) + intègre `<SalleModerationPanel>` en sidebar conditionnelle.
- 001-admin-salles-publiques (Phases 1→6 livrées) : workflow complet de propositions communautaires de salles Afrolang + nomination/révocation des administrateurs de salle publique. BDD : 2 nouvelles tables `afrolang.proposition_salle` (4 CHECK + 3 indexes dont unique partiel `auteur_id+groupe_ethnique_id WHERE statut='en_attente'`) et `afrolang.salle_administrateur` (CHECK `ck_admin_revocation_coherente` + 3 indexes dont unique partiel actif), 1 enum `statut_proposition_salle`, FK cross-schema vers `iam.utilisateur` et `country_profile.groupe_ethnique`. Backend : helper `est_administrateur_salle` (FR-019) ; handlers publics `soumettre_proposition` (rate-limit 5 rejets/7j → 429), `lister_mes_propositions`, `retirer_ma_proposition`, `lister_pays_disponibles` ; nouveau handler admin `admin/propositions_salle.rs` (4 endpoints — liste filtrable, détail, validation atomique en transaction `SELECT FOR UPDATE` qui crée la salle + lignes pays + bascule la proposition + audit `VALIDATE`+`CREATE`+notification, rejet avec commentaire ≥ 10 car.) ; 3 endpoints admin nomination/révocation/historique dans `admin/salles.rs` + 2 helpers cascade `cascader_suspendre_admins_salle` (FR-021, déclenché à `supprimer_salle` et `modifier_salle` avec actif=false) et `cascader_suspendre_admins_utilisateur` (FR-022, déclenché à `changer_etat_utilisateur` et `supprimer_utilisateur`). `lister_salles`/`obtenir_salle` enrichis du champ `administrateurs` (json_agg actif=TRUE). Notifications dédiées : `afrolang.proposition_validee`, `afrolang.proposition_refusee`, `afrolang.admin_salle_nomme`, `afrolang.admin_salle_revoque`. Frontend : composables `useAdminPropositionsSalle.ts` (CRUD propositions admin), extension `useAdminAfrolangSalles` (interface `SalleAdministrateurAPI` + `listerAdministrateurs`/`nommerAdministrateur`/`revoquerAdministrateur`) ; composants admin daisyUI `PropositionRow.vue` + `PropositionDetail.vue` (modales valider/rejeter avec validation commentaire) + `SalleAdministrateursPanel.vue` (recherche utilisateur, modal motif révocation, historique avec lignes inactives grisées) ; composant public Tailwind v4 pur `SalleAdministrateursWidget.vue` (chips ★ chocolat, distinct visuellement du rôle « Admin plateforme ») intégré à `SalleCard.vue` ; pages admin `pages/admin/afrolang/propositions/index.vue` (filtrage statut/langue/dates + pagination) et `[id].vue` (détail + actions) ; nouvel onglet « Administrateurs de salle » sur `pages/admin/salles/[id].vue`.
